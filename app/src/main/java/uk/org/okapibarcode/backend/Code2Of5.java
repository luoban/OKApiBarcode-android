    ...
    protected void plotSymbol() {
        ...
        for (xBlock = 0; xBlock < pattern[0].length(); xBlock++) {
            ...
            if (black) {
                ...
                if (w != 0 && h != 0) {
                    RectF rect = new RectF((float) (x + offset),(float) y, (float) w,(float) h);
                    rectangles.add(rect);
                }
        ...
        if (mode == ToFMode.ITF14) {
            // Add bounding box
            RectF topBar = new RectF(0, baseY, symbol_width, 4);
            RectF bottomBar = new RectF(0, baseY + symbol_height - 4, symbol_width, 4);
            RectF leftBar = new RectF(0, baseY, 4, symbol_height);
            RectF rightBar = new RectF(symbol_width - 4, baseY, 4, symbol_height);
